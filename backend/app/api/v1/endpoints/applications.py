...
async def get_applications(user=Depends(require_roles(UserRole.ADMIN, UserRole.COOK))):
    pass


@applications_router.post('/', summary='Создать заявку на закупку', description='Повар создает заявку на закупку продуктов')
async def create_application(user=Depends(require_roles(UserRole.ADMIN, UserRole.COOK))):
    pass


@applications_router.get('/{application_id}', summary='Получить заявку', description='Доступно поварам и администраторам')
async def get_application(application_id: int, user=Depends(require_roles(UserRole.ADMIN, UserRole.COOK))):
    pass
...
async def approve_application(application_id: int, user=Depends(require_roles(UserRole.ADMIN))):
    pass
...
async def reject_application(application_id: int, user=Depends(require_roles(UserRole.ADMIN))):
    pass