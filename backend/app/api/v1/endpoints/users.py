...
async def get_current_user(user=Depends(require_roles('student', 'admin', 'cook'))):
    return user


@users_router.patch("/me", summary="Обновить профиль текущего пользователя", description="")
async def update_current_user(user=Depends(require_roles('student', 'admin', 'cook'))):
    pass


@users_router.get("/me/allergies", summary="Получить список аллергий текущего пользователя", description="")
async def get_current_user_allergies(user=Depends(require_roles('student', 'admin', 'cook'))):
    pass


@users_router.post("/me/allergies", summary="Добавить аллергию", description="Указать аллерген для текущего пользователя")
async def add_current_user_allergy(user=Depends(require_roles('student', 'admin', 'cook'))):
    pass


@users_router.delete("/me/allergies", summary="Удалить аллергию", description="Удалить аллерген из списка")
async def delete_current_user_allergy(user=Depends(require_roles('student', 'admin', 'cook'))):
    pass
...
async def get_user(user_id: int, user=Depends(require_roles('admin'))):  
    pass


@users_router.patch("/{user_id}", summary="Обновить информацию о пользователе", description="Доступно только администраторам")
async def update_user(user_id: int, user=Depends(require_roles('admin'))): 
    pass