...
async def get_orders(user=Depends(require_roles('student', 'cook', 'admin'))):
    pass


@orders_router.post('/', summary='Создать заказ', description='Оплата разового питания учеником')
async def create_order(user=Depends(require_roles('student', 'admin'))):
    pass


@orders_router.get('/{order_id}', summary='Получить информацию о заказе', description='')
async def get_order(order_id: int, user=Depends(require_roles('student', 'cook', 'admin'))):
    pass
...
async def mark_seved_order(order_id: int, user=Depends(require_roles('student', 'admin'))):
    pass


@orders_router.post('/{order_id}/cancel', summary='Отменить заказ', description='Отмена заказа (доступно только до момента выдачи)')
async def cancel_order(order_id: int, user=Depends(require_roles('student', 'admin', 'cook'))):
    pass
...
async def confirm_order(order_id: int, user=Depends(require_roles('student', 'admin'))):
    pass


@orders_router.post('/{order_id}/serve', summary='Отметить заказ как выданный', description='Повар отмечает заказ как выданный.')
async def serve_order(order_id: int, user=Depends(require_roles('cook', 'admin'))):
    pass
